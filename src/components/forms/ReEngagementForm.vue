<template>
  <form @submit.prevent="createReEngagementTouchpoint">
    <BaseSelect id="reason-stopped" label="Why Did Client Stop Using Truck.me?" v-model="reasonStopped"
      :options="ReasonStoppedOptions" placeholder="Select a reason" :required="true" />

    <ReasonBranchFields :branch="reasonBranch" v-model:competitor-name="competitorName"
      v-model:why-competitor-won="whyCompetitorWon" ref="reasonBranchRef" />

    <BaseMultiSelect id="what-can-bring-back" label="What Can Bring Client Back?" v-model="whatCanBringBack"
      :options="WhatCanBringBackOptions" :required="true" />

    <BaseMultiSelect id="target-service" label="Target Service" v-model="targetService" :options="TargetServiceOptions"
      :required="true" />

    <BaseSelect id="return-potential" label="Return Potential" v-model="returnPotential"
      :options="ReturnPotentialOptions" placeholder="Select return potential" :required="true" />

    <BaseSelect id="re-engagement-status" label="Re-engagement Status" v-model="reEngagementStatus"
      :options="ReEngagementStatusOptions" placeholder="Select current status" :required="true" />

    <BaseSelect id="is-closing" label="Closing Out This Re-engagement?" v-model="isClosingReEngagement"
      :options="YesNoOptions" placeholder="Select an option" :required="true" />

    <template v-if="isClosingReEngagement === 'Yes'">
      <BaseSelect id="re-engagement-outcome" label="Re-engagement Outcome" v-model="reEngagementOutcome"
        :options="ReEngagementOutcomeOptions" placeholder="Select outcome" :required="true" />

      <BaseMultiSelect v-if="reEngagementOutcome === 'Reactivated'" id="what-brought-back"
        label="What Brought Client Back?" v-model="whatBroughtClientBack" :options="WhatBroughtClientBackOptions"
        :required="true" />

      <template v-if="reEngagementOutcome === 'Not Now'">
        <BaseSelect id="why-not-now" label="Why Not Now?" v-model="whyNotNow" :options="WhyNotNowOptions"
          placeholder="Select a reason" :required="true" />
      </template>

      <template v-if="reEngagementOutcome === 'Lost To Competitor'">
        <BaseInput id="competitor-name-lost" label="Competitor Name" v-model="competitorName" :required="true" />

        <BaseSelect id="why-competitor-won" label="Why Competitor Won?" v-model="whyCompetitorWon"
          :options="CompetitorReasonOptions" placeholder="Select a reason" :required="true" />
      </template>
    ...
    <BaseInput v-if="!opensChurnedForm" id="next-action-date" label="Next Action date" v-model="nextActionDate"
